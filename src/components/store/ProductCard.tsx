...
export function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-[16px] border border-[var(--border)] bg-white shadow-soft transition duration-200 hover:-translate-y-1 hover:border-[rgba(17,155,181,0.45)] hover:shadow-[0_26px_56px_rgba(18,63,109,0.13)] sm:rounded-[24px]">
      <Link
        className="relative block aspect-[16/11] overflow-hidden bg-[linear-gradient(180deg,#f8fbfc_0%,#edf6f8_100%)] min-[390px]:aspect-[4/3] sm:aspect-square"
      ...
      <div className="flex flex-1 flex-col p-3 min-[390px]:p-2.5 sm:p-4">
        <Link href={`/product/${product.slug}`}>
          <h3 className="line-clamp-2 min-h-10 text-[13px] font-black leading-5 text-[var(--text)] transition group-hover:text-[var(--primary)] min-[390px]:text-xs min-[390px]:leading-5 sm:min-h-12 sm:text-base sm:leading-6">
            {product.name}
          </h3>
        </Link>
        <div className="mt-1.5 flex flex-wrap items-center gap-1 text-[10px] font-black sm:mt-2 sm:gap-2 sm:text-xs">
        ...
        <div className="mt-auto flex flex-col gap-1 pt-2.5 sm:gap-2 sm:pt-4">
          <button
            className="btn-primary w-full text-[11px] min-[360px]:text-xs sm:text-sm"
